...
class NilaiMahasiswa extends BaseAPI
{
	...
	public function index()
	{
		if ($this->input->server('REQUEST_METHOD') === 'GET') {
			...
			$daftar_nilai = $this->db
				->select('nilai_mahasiswa.id as id, mahasiswa.nama, mahasiswa.nim')
				...
				->where('pembimbing_industri.nik', $this->pem->nik)
	...
	public function edit($id)
	{
		if ($this->input->server('REQUEST_METHOD') === 'POST') {
			$kemampuan_kerja = $this->input->post('kemampuan_kerja');
			...
			if (!$kemampuan_kerja || !$disiplin || !$komunikasi || !$inisiatif || !$kreativitas || !$kerjasama) {
				die(
					json_encode([
						'success' => false,
						'message' => 'Bad Request.',
